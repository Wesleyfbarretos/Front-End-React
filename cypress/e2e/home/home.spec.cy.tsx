...
describe("Visit my home page", () => {
  ...
  it("should open and close the dex", () => {
	...
	it('should check if the filter by types is working and finally reset it', () => {
		...
		cy.wrap([cardArray]).each((index: number) => cy.get('.card').eq(index).should('have.class', 'card').and('have.css', 'background-color', 'rgb(102, 217, 255)'))
		...
		cy.get('#card-place').find('.card').its('length').then((len) => expect(len).to.equal(24))
	})
	...
	it('should check if the filter by name is working and finally reset it', () => {
		...
		cy.get('#card-place').find('.card').its('length').then((len) => expect(len).to.equal(1))
	...
	it('should check if infinite scrolling to fetch more cards is working', () => {
		...
		cy.wait('@fetch').then(() => {
			...
			cy.get('#card-place').find('.card').its('length').then((len) => {	expect(len).to.equal(inicialLength)})
		...
		cy.wait('@fetch2').then(() => {
			cy.wait(500)
			cy.get('#card-place').find('.card').its('length').then((len) => {	expect(len).to.equal(inicialLength * 2)})
			cy.window().scrollTo('bottom')
		})
		...
		cy.wait('@fetch3').then(() => {
			cy.wait(500)
			cy.get('#card-place').find('.card').its('length').then((len) => {	expect(len).to.equal(inicialLength * 3)})
			cy.window().scrollTo('bottom')
		})
		...
		cy.wait('@fetch4').then(() => {
			cy.wait(500)
			cy.get('#card-place').find('.card').its('length').then((len) => {	expect(len).to.equal(inicialLength * 4)})
		})
